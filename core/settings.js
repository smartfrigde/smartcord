...
const scSettingsID = require("path").parse(__filename).name;
...
module.exports = new Plugin({
  ...
  color: "darkred",
  async load() {
    const discordConstants = SCApi.findModule("API_HOST");
    const UserSettings = module.exports.utils.getComponentFromFluxContainer(
      SCApi.findModule("getUserSettingsSections").default
    );
    ...
    if (!SC.discordComponents) {
      SC.discordComponents = {};
    }

    this._initClassMaps(SC.classMaps);
    this._initDiscordComponents(SC.discordComponents);
    ...
    this.unpatch = SCApi.monkeyPatch(
  ...
  utils: {
    ...
    shouldPluginRender(id) {
      ...
      if (
        SC.plugins[id].getSettingsPanel &&
        typeof SC.plugins[id].getSettingsPanel == "function"
      ) {
        shouldRender = false;
      }

      if (
        SC.plugins[id].settings.enabled === false ||
        !SC.plugins[id].generateSettings
      ) {
        shouldRender = false;
      }
    ...
    return [
      {
        ...
        element: () => {
          ...
          const { header } = SCApi.findModule("topPill");

          return SCApi.React.createElement(
            "div",
  ...
  _getPluginSections() {
    const arr = [];
    for (const key in SC.plugins) {
      if (typeof SC.plugins[key].generateSettingsSection !== "function")
        continue;

      const label = SC.plugins[key].settingsSectionName || SC.plugins[key].name;
      arr.push({
        section: "SC/" + key,
        label,
        element: () =>
          SCApi.React.createElement(this.components.PluginSection, {
            id: key,
            label,
          }),
      });
  ...
  _initClassMaps(obj) {
    const divM = SCApi.findModule(
      (m) => m.divider && Object.keys(m).length === 1
    );
    obj.headers = SCApi.findModule("defaultMarginh2");
    obj.margins = SCApi.findModule("marginBottom8");
    obj.divider = divM ? divM.divider : "";
    obj.checkbox = SCApi.findModule("checkboxEnabled");
    obj.buttons = SCApi.findModule("lookFilled");
    obj.switch = SCApi.findModule("switch");
    obj.alignment = SCApi.findModule("horizontalReverse");
    obj.description = SCApi.findModule("formText");
    // New
    obj.shadows = SCApi.findModule("elevationHigh");
  },
  _initDiscordComponents(obj) {
    obj.Textbox = SCApi.findModuleByDisplayName("TextInput");
    obj.Select = SCApi.findModuleByDisplayName("SelectTempWrapper");
    obj.RadioGroup = SCApi.findModuleByDisplayName("RadioGroup");
    obj.Title = SCApi.findModuleByDisplayName("FormTitle");
    obj.Text = SCApi.findModuleByDisplayName("FormText");
    obj.FormSection = SCApi.findModuleByDisplayName("FormSection");
    obj.Icon = SCApi.findModuleByDisplayName("Icon");
    obj.LoadingSpinner = SCApi.findModuleByDisplayName("Spinner");
    obj.Card = SCApi.findModuleByDisplayName("FormNotice");
    obj.Flex = SCApi.findModuleByDisplayName("Flex");
    obj.Switch = SCApi.findModuleByDisplayName("Switch");
    obj.SwitchItem = SCApi.findModuleByDisplayName("SwitchItem");
    obj.Slider = SCApi.findModuleByDisplayName("Slider");
    obj.Select = SCApi.findModuleByDisplayName("SelectTempWrapper");
    obj.Tooltip = SCApi.findModuleByDisplayName("Tooltip");
    obj.Button = SCApi.findModule("Sizes");
    ...
    obj.Divider = (props) => {
      props.className = props.className
        ? props.className + " " + SC.classMaps.divider
        : SC.classMaps.divider;
      return SCApi.React.createElement("div", Object.assign({}, props));
    };
  },
  _initReactComponents() {
    ...
    const PluginsPage = () => {
      return e(
        ...
        Object.keys(SC.plugins).map((id) =>
          e(PluginListing, { id, plugin: SC.plugins[id] })
        )
      );
    };
    ...
    class PluginListing extends Component {
      ...
      handleLoad() {
        ...
        SC.plugins[this.props.id].settings = this.props.plugin.settings;
    ...
    const PluginSettings = (props) => {
      const plugin = SC.plugins[props.id];
    ...
    const DiscordUIGenerator = {
      reactMarkdownRules: (() => {
        const simpleMarkdown = SCApi.findModule("markdownToReact");
      ...
      _parseMD(content) {
        ...
        const { markdownToReact } = SCApi.findModule("markdownToReact");

        return markdownToReact(content, reactMarkdownRules);
      },
      _loadData(props) {
        const plug = SC.plugins[props.pluginID];
        if (!plug) return;
        if (plug.customLoad) {
          // custom function for loading settings
          return plug.customLoad(props.configName);
        }
        return SCApi.loadData(props.pluginID, props.configName);
      },
      _saveData(props, data) {
        ...
        const r = SCApi.saveData(props.pluginID, props.configName, data);
