...
function saveAs(url, filename, fileExtension) {
  dialog
    .showSaveDialog({
      ...
      title: "Where would you like to store the stolen memes?",
      buttonLabel: "Steal this meme",
      filters: [{ name: "Stolen meme", extensions: [fileExtension] }],
    })
    .then((e) => {
      if (!e.canceled) {
...
module.exports = new Plugin({
  ...
  listener(e) {
    if (document.getElementsByClassName(this._cmClass).length == 0)
      setTimeout(() => module.exports.onContextMenu(e), 0);
  ...
  onContextMenu(e) {
    ...
    if (
      ...
      !messageGroup &&
      !e.target.className.includes(guildWrapper) &&
      !parentElem.className.includes(memberAvatar) &&
    ...
    let saveLabel = "Download",
      url =
        e.target.poster ||
        e.target.style.backgroundImage.substring(
          e.target.style.backgroundImage.indexOf(`'`) + 1,
          e.target.style.backgroundImage.lastIndexOf(`'`)
        ) ||
        e.target.href ||
        e.target.src;
    ...
    if (e.target.className.includes(guildWrapper)) {
      ...
      if (e.target.firstChild.src) {
        // Make sure guild box has an icon
        url = e.target.firstChild.src;
      }
    } else if (
      e.target.className.includes("avatar-") ||
      (parentElem.nodeName == "DIV" &&
        parentElem.className.includes(memberAvatar))
    ) {
    ...
    if (
      !url ||
      e.target.classList.contains("emote") ||
    ...
    if (saveLabel.includes("Avatar") || saveLabel.includes("Icon"))
      url += "?size=2048";
